...
describe ChampionMasteryRequest do
  ...
  describe "#total_score" do
    it "returns the total score" do
      stub_request_raw subject, 60, "scores/by-summoner/#{encrypted_id}"
      expect(subject.total_score encrypted_summoner_id: encrypted_id).to eq 60
    end
  end

  describe "#find" do
    it "returns a ChampionMastery" do
      stub_request(subject, 'champion-mastery', "champion-masteries/by-summoner/#{encrypted_id}/by-champion/40")
      expect(subject.find 40, encrypted_summoner_id: encrypted_id).to be_a DynamicModel
    end

    it "fetches ChampionMastery from the API" do
      stub_request(subject, 'champion-mastery', "champion-masteries/by-summoner/#{encrypted_id}/by-champion/40")
      result = subject.find 40, encrypted_summoner_id: encrypted_id
  ...
  describe "#all" do
    before { stub_request(subject, 'champion-masteries', "champion-masteries/by-summoner/#{encrypted_id}") }
    let(:result) { subject.all encrypted_summoner_id: encrypted_id }
